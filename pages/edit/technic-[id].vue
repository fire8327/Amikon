<template>
    <div class="flex flex-col gap-6">
        <div class="flex items-center justify-between">
            <p class="mainHeading">Изменение оборудования</p>
        ...
        <FormKit @submit="addEntity" type="form" :actions="false" messages-class="hidden" form-class="flex flex-col gap-6 items-center justify-center grow">
            ...
            <div class="flex items-center lg:items-start gap-4 max-lg:flex-col w-full md:w-2/3 lg:w-1/2">
                ...
                <FormKit v-if="entityForm.type == 'ПО'" v-model="entityForm.model" validation="required" messages-class="text-[#E9556D] font-mono" type="text" placeholder="Модель" name="Модель" outer-class="w-full lg:w-1/3" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-violet-500 shadow-md"/>
            ...
            <FormKit v-model="entityForm.status" validation="required" messages-class="text-[#E9556D] font-mono" type="select" :options="['Отключено','В работе','Сбой','Обслуживание']" placeholder="Состояние" name="Состояние" outer-class="w-full md:w-2/3 lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-violet-500 shadow-md"/>
            ...
            <button :disabled="isLoading" :class="{ 'opacity-50 cursor-not-allowed': isLoading }" type="submit" class="px-4 py-2 border border-violet-500 bg-violet-500 text-white rounded-full w-[160px] text-center transition-all duration-500 hover:text-violet-500 hover:bg-transparent">Обновить</button>
        </FormKit>
    </div>
</template>
...
const addEntity = async() => {
    ...
    try {
        ...
        .from('technic')
        .update(entityForm.value)
        .eq('id', route.params.id)

        if (error) throw error

        showMessage('Оборудование обновлено!', true)
